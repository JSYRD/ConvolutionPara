...
int main(int argc, char *argv[])
{
	...
	int(*recv)[I_SIZE] = (int(*)[I_SIZE])malloc(sizeof(int)*segment*I_SIZE);
	...
	//这样就可以避免给所有的进程进行赋值运算
	MPI_Scatter(&I[0][0], segment*I_SIZE, MPI_INT, &recv[0][0], segment * I_SIZE, MPI_INT, 0, MPI_COMM_WORLD);
	...
	for (int batch = 0; batch < BATCHSIZE; batch++){
		for (y = 0; y<segment; y++) {
			for (x = 0; x<I_SIZE; x++) {
				temp[y][x] = convolution(recv, h, y, x);               // Get convolution value in point y, x
...
int convolution(int (*I)[I_SIZE], int h[H_SIZE][H_SIZE], int y, int x)//含有padding,输入矩阵和输出矩阵大小相等
{
...
void save_to_file(int A[I_SIZE][I_SIZE])
{
	...
	file_conv_res = fopen("convolution_z2_opt2200.txt", "w");
